...
AU_info <- arc.select(arc.open('https://services.arcgis.com/uUvqNMGPm7axC2dD/arcgis/rest/services/Oregon_AU/FeatureServer/4')) |>
  select(AU_ID, AU_Name,AU_UseCode, HUC12) |>
  distinct() |>
  group_by(AU_ID, AU_Name, AU_UseCode) |>
  summarise(HUC12 = as.numeric(min(HUC12)))
